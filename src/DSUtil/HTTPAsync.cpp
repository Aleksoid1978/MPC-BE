...
static CString GetLastErrorMessage(DWORD dwError)
{
	CString errMsg;
...
{ \
	...
	if (dwError != ERROR_IO_PENDING) { \
		DbgLog((LOG_TRACE, 3, L"CHTTPAsync() error : Function '%s' failed with error %d - '%s', %s : %i", CString(lpszFunction), dwError, GetLastErrorMessage(dwError), GetFileOnly(CString(__FILE__)), __LINE__)); \
		return (ret); \
	} \
} \
