...
static void SaveDefaultCodec(std::vector<Codec>& codecs, CComboBox& box, const GUID& cat)
{
	if (cat == GUID_NULL) {
		return;
	}
...
static int ShowPPage(std::vector<Codec>& codecs, CComboBox& box, HWND hWnd = nullptr)
{
	int iSel = box.GetCurSel();
	if (iSel < 0) {
		return(-1);
