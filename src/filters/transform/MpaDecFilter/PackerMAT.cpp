...
void CPackerMAT::SetCallback(Callback_t&& callback)
{
...
void CPackerMAT::PackTrueHD(const uint8_t* data, int size, bool syncFrame)
{
  ...
  {
    DLog(L"CPackerMAT::PackTrueHD: seek detected, re-initializing MAT packer state");
    m_state = {};
    m_state.init = true;
    m_buffer.clear();
    m_bufferCount = 0;
...
void CPackerMAT::FlushPacket()
{
  ...
  // push MAT packet data to callback
  m_callback(m_buffer.data());
