...
//
// from LAVFilters/decoder/LAVVideo/pixconv/yuv444_ayuv.cpp
//
...
template <MPCPixFmtType inputFormat, int shift, int outFmt, int right_edge, int ycgco>
                                     ...
                                     const RGBCoeffs *coeffs, const uint16_t *dithers)
{
    ...
    if (inputFormat == PFType_YUV420 || inputFormat == PFType_NV12 || inputFormat == PFType_P010)
    {
        if (line == 0)
        {
            ...
            {
                yuv2rgb_convert_pixels<inputFormat, shift, outFmt, 0, ycgco>(y, u, v, rgb, 0, 0, 0, line,
                                                                                         coeffs, lineDither, i);
            }
            yuv2rgb_convert_pixels<inputFormat, shift, outFmt, 1, ycgco>(y, u, v, rgb, 0, 0, 0, line,
                                                                                     coeffs, lineDither, 0);
    ...
    for (; line < lastLine; line += 2)
    {
        ...
        for (ptrdiff_t i = 0; i < endx; i += 4)
        {
            yuv2rgb_convert_pixels<inputFormat, shift, outFmt, 0, ycgco>(
                y, u, v, rgb, srcStrideY, srcStrideUV, dstStride, line, coeffs, lineDither, i);
        }
        yuv2rgb_convert_pixels<inputFormat, shift, outFmt, 1, ycgco>(
...
HRESULT CFormatConverter::convert_yuv_rgb(const uint8_t* const src[4], const ptrdiff_t srcStride[4], uint8_t* dst[], int width, int height, const ptrdiff_t dstStride[])
{
    ...
    int outFmt = 1; // RGB32
    const uint16_t *dithers = nullptr; // LAVDither_Ordered
    ...
    switch (inputFormat) {
    case PFType_YUV420Px: inputFormat = PFType_YUV420; break;
    case PFType_YUV422Px: inputFormat = PFType_YUV422; break;
    case PFType_YUV444Px: inputFormat = PFType_YUV444; break;
    }
  ...
  ptrdiff_t dstStride0 = dstStride[0];
  if (m_out_pixfmt == PixFmt_RGB32 && m_OutHeight > 0) {
    // flip the image, if necessary
    dst[0] += dstStride0 * (height - 1);
    dstStride0 = -dstStride0;
  }

    // run conversion, threaded
    if (m_NumThreads <= 1)
    {
        convFn(src[0], src[1], src[2], dst[0], width, height, srcStride[0], srcStride[1], dstStride0, 0, height,
               coeffs, dithers);
    }
    else
    {
        ...
        Concurrency::parallel_for(0, m_NumThreads, [&](int i) {
            ...
            convFn(src[0], src[1], src[2], dst[0], width, height, srcStride[0], srcStride[1], dstStride0,
