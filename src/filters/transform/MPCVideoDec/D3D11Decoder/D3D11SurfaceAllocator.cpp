...
CD3D11SurfaceAllocator::CD3D11SurfaceAllocator(CD3D11Decoder* pDec, HRESULT *phr)
    : CBaseAllocator(L"CD3D11SurfaceAllocator", nullptr, phr)
    , m_pDec(pDec)
{
}
...
{
    DLog(L"CD3D11SurfaceAllocator::Alloc()");
    HRESULT hr = S_OK;
...
STDMETHODIMP CD3D11SurfaceAllocator::ReleaseBuffer(IMediaSample *pSample)
{
    ...
    {
        DLog(L"CD3D11SurfaceAllocator::ReleaseBuffer: Freeing late sample");
        delete pD3D11Sample;
        return S_OK;
    }
    else
