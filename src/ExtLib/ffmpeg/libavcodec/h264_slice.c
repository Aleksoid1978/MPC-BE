...
static int alloc_picture(H264Context *h, H264Picture *pic)
{
    ...
    if (CONFIG_GRAY && !h->avctx->hwaccel &&
        ...
        !h->avctx->using_dxva &&
        // ==> End patch MPC
        h->flags & CODEC_FLAG_GRAY && pic->f->data[2]) {
...
static void fill_dxva_slice_long(H264Context *h)
{
    ...
    pSlice->luma_log2_weight_denom       = sl->luma_log2_weight_denom;
    pSlice->chroma_log2_weight_denom     = sl->chroma_log2_weight_denom;
    pSlice->slice_alpha_c0_offset_div2   = sl->slice_alpha_c0_offset / 2 - 26;
    pSlice->slice_beta_offset_div2       = sl->slice_beta_offset     / 2 - 26;

    pSlice->num_ref_idx_l0_active_minus1 = 0;
    pSlice->num_ref_idx_l1_active_minus1 = 0;
    if (sl->list_count > 0) {
        pSlice->num_ref_idx_l0_active_minus1 = sl->ref_count[0] - 1;
    }
    if (sl->list_count > 1) {
        pSlice->num_ref_idx_l1_active_minus1 = sl->ref_count[1] - 1;
    }
    ...
    pSlice->slice_qp_delta    = sl->qscale - h->pps.init_qp;;
    pSlice->redundant_pic_cnt = sl->redundant_pic_count;
    if (sl->slice_type == AV_PICTURE_TYPE_B)
        pSlice->direct_spatial_mv_pred_flag = sl->direct_spatial_mv_pred;
    pSlice->cabac_init_idc = h->pps.cabac ? sl->cabac_init_idc : 0;
    if (sl->deblocking_filter < 2)
        pSlice->disable_deblocking_filter_idc = 1 - sl->deblocking_filter;
    else
